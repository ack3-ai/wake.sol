...
error classes. Running this module emits, deterministically:
  ...
  * ``wake_sol/_errors_catalog/{solana,anchor,system,token}.py`` — one real
...
Run ``python -m wake_sol._gen.errors_catalog`` to regenerate, or with
...
_HEADER = "# @generated by wake_sol._gen.errors_catalog — do not edit.\n"
...
def _emit_init() -> str:
    return (
        ...
        '``wake_sol._gen.errors_catalog`` — do not edit."""\n'
    )
    ...
    if check:
        ...
        if stale:
            ...
            print(f"[errors_catalog] stale/missing: {names}\n"
                  f"  run: python -m wake_sol._gen.errors_catalog", file=sys.stderr)
            return 2
